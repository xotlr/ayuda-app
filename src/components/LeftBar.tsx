...
export const LeftBar = ({ selectedTab }: { selectedTab: Tab | null }) => {
  ...
  return (
    <>
      <nav className="fixed bottom-0 left-0 top-0 hidden flex-col gap-5 border-r-2 border-gray-200/10 bg-muted p-3 md:flex lg:w-64 lg:p-5">
        <Link
          href="/learn"
        ...
        <ul className="flex flex-col items-stretch gap-2">
            ...
            return (
              <li key={item.href} className="flex flex-1">
                {item.name === selectedTab ? (
                  <Link
                    href={item.href}
                    className="flex grow items-center gap-3 rounded-xl border-2 border-gray-200/10 bg-muted p-4 py-3 tracking-wide text-sm font-bold uppercase text-blue-400"
                  >
                    {item.icon}{" "}
                  ...
                  <Link
                    href={item.href}
                    className="flex grow items-center gap-3 rounded-xl border-2 bg-muted p-4 py-3 tracking-wide text-sm font-bold uppercase"
                  >
                    {item.icon}{" "}
                    <span className="sr-only lg:not-sr-only">{item.name}</span>
                  </Link>
          ...
          <div
            className="relative flex grow items-center gap-3 rounded-xl border-2 bg-muted p-4 py-3 tracking-wide text-sm font-bold uppercase cursor-default"
          ...
          >
            ...
            <div
              className={[
                "absolute left-full top-[-10px] min-w-[300px] rounded-2xl border-2 border-gray-200/10 bg-muted text-left text-muted-foreground",
                moreMenuShown ? "" : "hidden",
              ].join(" ")}
            >
              <div className="flex flex-col py-2">
                <Link
                  className="flex items-center gap-4 px-5 py-2 text-left uppercase hover:bg-gray-200/10"
              ...
              <div className="flex flex-col border-t-2 border-gray-300 py-2">
                {!loggedIn && (
                  <button
                    className="px-5 py-2 text-left uppercase hover:bg-gray-200/10"
